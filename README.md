...
ADD - это более старая инструкция, которая может загружать и извлекать файлы, а не копировать их в изображения с хоста. COPY может только копроватьфайлы и каталоги. Если вы хотите загрузить или извлечь файл, то стаайтесь использоватьинструкцию `RUN` с обычными командами Linux для загрузки и извлечения файлов.
