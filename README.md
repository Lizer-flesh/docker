...
Инструкция `LABEL` включает в себя контактные данные создателяя образа. Объявление меток не замедляет процесс сборки образа и не увеличивает его размер. Они лишь содержат полезную информацию об образе. Поэтому их рекомендуется включать в файл.
