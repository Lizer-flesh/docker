...
Поэтому регомендуется использовать многоэтапные сбоорки. При их использовании спользуется несколько FROM операторов в файле Dockerfile. Каждая инструкция FROM может использовать разную бау и каждая из них начинает новый этап сборки. Вы можете выборочно копировать артефакты с одного этапа на другой, не включая ненужные вещи в финальный image.
